...
def get_next_pos(board, pos):
    ...
    dx = [0, 1, 0, -1]
    for i in range(4):
        if board[pos1_y + dy[i]][pos1_x + dx[i]] == 0 and board[pos2_y + dy[i]][pos2_x + dx[i]] == 0:
            step = set([(pos1_y + dy[i], pos1_x + dx[i]), (pos2_y + dy[i], pos2_x + dx[i])])
            next_pos.append(step)
    if pos1_y == pos2_y:
        for i in [-1, 1]:
            if board[pos1_y + i][pos1_x] == 0 and board[pos2_y + i][pos2_x] == 0:
                step = set([(pos1_y, pos1_x), (pos1_y + i, pos1_x)])
                next_pos.append(step)
                step = set([(pos2_y, pos2_x), (pos2_y + i,pos2_x)])
                next_pos.append(step)
    if pos1_x == pos2_x:
        for i in [-1, 1]:
            if board[pos1_y][pos1_x + i] == 0 and board[pos2_y][pos2_x + i] == 0:
                step = set([(pos1_y, pos1_x), (pos1_y, pos1_x + i)])
                next_pos.append(step)
                step = set([(pos2_y, pos2_x), (pos2_y, pos2_x + i)])
                next_pos.append(step)
    return next_pos
